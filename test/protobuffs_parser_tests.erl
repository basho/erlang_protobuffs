...
message_test_() ->
    ...
    Parsed = protobuffs_parser:parse(Result),
    [?_assertMatch({ok,[{message, "Test",[{1,required,"string","name",number,none}]}]},Parsed)].

message_default_test_() ->
    {ok,Result,1} = protobuffs_scanner:string("message Test { optional float value = 1 [default=0.01]; }"),
    Parsed = protobuffs_parser:parse(Result),
    [?_assertMatch({ok,[{message, "Test",[{1,optional,"float","value",number,0.01}]}]},Parsed)].

packed_test_() ->
    {ok,Result,1} = protobuffs_scanner:string("message Test { repeated float values = 1 [packed=true]; }"),
    Parsed = protobuffs_parser:parse(Result),
    [?_assertMatch({ok,[{message, "Test",[{1,repeated_packed,"float","values",number,none}]}]},Parsed)].
...
extend_test_() ->
    ...
    Parsed = protobuffs_parser:parse(Result),
    [?_assertMatch({ok,[{extend, "Foo", [{126, optional, "int32", "bar", number, none}]}]}, Parsed)].
